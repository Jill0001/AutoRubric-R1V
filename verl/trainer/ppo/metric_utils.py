...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Add reward components metrics if available
    # Check for reward components directly in non_tensor_batch (they are added as individual keys)
    if "rule_score" in batch.non_tensor_batch:
        rule_scores = torch.tensor(
            batch.non_tensor_batch["rule_score"], device=sequence_reward.device
        )
        metrics["reward_components/rule_score/mean"] = (
            torch.mean(rule_scores).detach().item()
        )
        metrics["reward_components/rule_score/max"] = (
            torch.max(rule_scores).detach().item()
        )
        metrics["reward_components/rule_score/min"] = (
            torch.min(rule_scores).detach().item()
        )

    # Process llm_score component
    if "llm_score" in batch.non_tensor_batch:
        llm_scores = torch.tensor(
            batch.non_tensor_batch["llm_score"], device=sequence_reward.device
        )
        metrics["reward_components/llm_score/mean"] = (
            torch.mean(llm_scores).detach().item()
        )
        metrics["reward_components/llm_score/max"] = (
            torch.max(llm_scores).detach().item()
        )
        metrics["reward_components/llm_score/min"] = (
            torch.min(llm_scores).detach().item()
        )

        # Calculate consistency rate (percentage of 1.0 scores for binary consistency)
        consistency_rate = (llm_scores == 1.0).float().mean().detach().item()
        metrics["reward_components/llm_score/consistency_rate"] = consistency_rate

    # Add weights if available
    if "rule_weight" in batch.non_tensor_batch:
        rule_weight = batch.non_tensor_batch["rule_weight"]
        if len(rule_weight) > 0:
            metrics["reward_components/rule_weight"] = (
                rule_weight[0]
                if isinstance(rule_weight, (list, np.ndarray))
                else rule_weight
            )

    if "llm_weight" in batch.non_tensor_batch:
        llm_weight = batch.non_tensor_batch["llm_weight"]
        if len(llm_weight) > 0:
            metrics["reward_components/llm_weight"] = (
                llm_weight[0]
                if isinstance(llm_weight, (list, np.ndarray))
                else llm_weight
            )

    # Add raw rule and LLM score metrics as critic metrics (only mean values)
    if "rule_score" in batch.non_tensor_batch:
        rule_scores = torch.tensor(
            batch.non_tensor_batch["rule_score"], device=sequence_reward.device
        )
        metrics["critic/rewards_rule/mean"] = torch.mean(rule_scores).detach().item()

    if "llm_score" in batch.non_tensor_batch:
        llm_scores = torch.tensor(
            batch.non_tensor_batch["llm_score"], device=sequence_reward.device
        )
        metrics["critic/rewards_judge/mean"] = torch.mean(llm_scores).detach().item()

    # Add rubric score metrics
    if "rubric_score" in batch.non_tensor_batch:
        rubric_scores = torch.tensor(
            batch.non_tensor_batch["rubric_score"], device=sequence_reward.device
        )
        metrics["critic/rewards_rubric/mean"] = (
            torch.mean(rubric_scores).detach().item()
        )
        metrics["reward_components/rubric_score/mean"] = (
            torch.mean(rubric_scores).detach().item()
        )
        metrics["reward_components/rubric_score/max"] = (
            torch.max(rubric_scores).detach().item()
        )
        metrics["reward_components/rubric_score/min"] = (
            torch.min(rubric_scores).detach().item()
        )
    ...
    for data_source, var2metric2prompt_vals in data_src2var2metric2prompt_vals.items():
        for var_name, metric2prompt_vals in var2metric2prompt_vals.items():
            for metric_name, prompt_vals in metric2prompt_vals.items():
                ...
                valid_vals = [v for v in prompt_vals if v is not None]
                if valid_vals:
                    data_src2var2metric2val[data_source][var_name][metric_name] = (
                        np.mean(valid_vals)
                    )
                else:
