...
def _safe_num_proc(requested: Optional[int]) -> Optional[int]:
...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        default_workers = max(1, cpu_cnt // 4)  # 默认建议
        self.num_workers = config.get(
            "filter_overlong_prompts_workers", default_workers
        )
        self.num_workers = min(self.num_workers, cpu_cnt)

        self._num_proc_filter = _safe_num_proc(self.num_workers)
        self._download()
    ...
    def maybe_filter_out_long_prompts(self, dataframe: datasets.Dataset = None):
        ...
        if self.filter_overlong_prompts:
            ...
            if self._num_proc_filter is not None:
                kwargs["num_proc"] = (
                    self._num_proc_filter
                )  # 仅非-daemon 且 >1 时才会存在
    ...
    def __getitem__(self, item):
        ...
        instruction = "You FIRST think about the reasoning process as an internal monologue and then provide the final answer. The reasoning process MUST BE enclosed within <think> </think> tags. The final answer MUST BE put in \\boxed{}."

        has_added = False
        for msg in messages:
            if msg.get("role") == "user":
                content = msg["content"]
                if isinstance(content, str) and (not has_added):
                    item["text"] = item["text"] + "\n" + instruction
                    has_added = True
        ...
        # Extract problem_id from top level or extra_info
        # problem_id should already be at top level (it's loaded from parquet)
        # but we ensure it's passed through to the batch
        if "problem_id" not in row_dict and "problem_id" in extra_info:
            row_dict["problem_id"] = extra_info["problem_id"]

        # Extract answer from extra_info if available
        if "answer" in extra_info:
            row_dict["answer"] = extra_info["answer"]
