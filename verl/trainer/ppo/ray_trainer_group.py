...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if not isinstance(role_worker_mapping, dict):
            raise TypeError(
                f"role_worker_mapping must be a dict-like object, got {type(role_worker_mapping).__name__}."
            )

        normalized_role_mapping: dict[Role, WorkerType] = {}
        for key, worker_cls in role_worker_mapping.items():
            if isinstance(key, Role):
                normalized_key = key
            elif isinstance(key, str):
                try:
                    normalized_key = Role[key]
                except KeyError as exc:
                    raise ValueError(
                        f"Unsupported role key '{key}'. Expected one of {[role.name for role in Role]}."
                    ) from exc
            else:
                raise TypeError(
                    "role_worker_mapping keys must be instances of `Role` or their string names. "
                    f"Received key of type {type(key).__name__}."
                )

            normalized_role_mapping[normalized_key] = worker_cls

        self.role_worker_mapping = normalized_role_mapping
    ...
    def _validate_config(self):
        ...
        if config.data.train_batch_size is None:
            raise ValueError(
                "[validate_config] `data.train_batch_size` is missing. Please set a positive "
                "global prompt batch size via your config or CLI override."
            )
        if config.data.train_batch_size <= 0:
            raise ValueError(
                "[validate_config] `data.train_batch_size` must be a positive integer. "
                f"Received: {config.data.train_batch_size!r}."
            )
        ...
        if real_train_batch_size % minimal_bsz != 0:
            raise ValueError(
                "[validate_config] Invalid effective batch size. "
                f"`data.train_batch_size`={config.data.train_batch_size}, "
                f"rollout.n={config.actor_rollout_ref.rollout.n}, "
                f"minimal_bsz={minimal_bsz}. "
                "The product `data.train_batch_size * rollout.n` must be divisible by the minimal batch size "
                "determined by the parallelism configuration."
            )
        ...
        if not config.actor_rollout_ref.actor.use_dynamic_bsz:
            if (
                ...
                raise ValueError(
                    "[validate_config] `data.train_batch_size` must be greater than or equal to "
                    "`actor_rollout_ref.actor.ppo_mini_batch_size` when dynamic batching is disabled. "
                    f"Got train_batch_size={config.data.train_batch_size}, "
            ...
            if config.actor_rollout_ref.actor.ppo_micro_batch_size is not None:
                if (
                    ...
                    raise ValueError(
                        "[validate_config] `actor_rollout_ref.actor.ppo_mini_batch_size` must be divisible by "
                        "`actor_rollout_ref.actor.ppo_micro_batch_size`. "
                        f"Got ppo_mini_batch_size={config.actor_rollout_ref.actor.ppo_mini_batch_size}, "
                        f"ppo_micro_batch_size={config.actor_rollout_ref.actor.ppo_micro_batch_size}."
                    )
                if (
                    ...
                    raise ValueError(
                        "[validate_config] `actor_rollout_ref.actor.ppo_micro_batch_size * ulysses_sequence_parallel_size` "
                        "must cover at least the number of GPUs. "
                        f"Got micro_batch_size={config.actor_rollout_ref.actor.ppo_micro_batch_size}, "
                        f"sequence_parallel_size={sp_size}, n_gpus={n_gpus}."
                    )
        ...
        # critic
        if self.use_critic and not config.critic.use_dynamic_bsz:
            if config.data.train_batch_size < config.critic.ppo_mini_batch_size:
                raise ValueError(
                    "[validate_config] `data.train_batch_size` must be greater than or equal to "
                    "`critic.ppo_mini_batch_size` when critic dynamic batching is disabled. "
                    f"Got train_batch_size={config.data.train_batch_size}, "
                    f"critic_ppo_mini_batch_size={config.critic.ppo_mini_batch_size}."
                )
            ...
            if config.critic.ppo_micro_batch_size is not None:
                if (
                    ...
                    raise ValueError(
                        "[validate_config] `critic.ppo_mini_batch_size` must be divisible by "
                        "`critic.ppo_micro_batch_size`. "
                        f"Got critic_ppo_mini_batch_size={config.critic.ppo_mini_batch_size}, "
                        f"critic_ppo_micro_batch_size={config.critic.ppo_micro_batch_size}."
                    )
                if config.critic.ppo_micro_batch_size * sp_size < n_gpus:
                    raise ValueError(
                        "[validate_config] `critic.ppo_micro_batch_size * ulysses_sequence_parallel_size` must cover "
                        "at least the number of GPUs. "
                        f"Got critic_micro_batch_size={config.critic.ppo_micro_batch_size}, "
                        f"sequence_parallel_size={sp_size}, n_gpus={n_gpus}."
                    )
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if "agent_name" in batch.non_tensor_batch:
                    non_tensor_batch_keys_to_pop.append("agent_name")
                # Note: We intentionally DO NOT pop problem_id here as we need it for logging
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        # Add global_step and total_steps to extra_info for dynamic reward weighting
                        if "extra_info" not in batch.non_tensor_batch:
                            batch.non_tensor_batch["extra_info"] = [{}] * len(batch)
                    ...
                    if rollout_data_dir:
                        with marked_timer(
                            ...
                            # Preserve the reward_extra_infos_dict from compute_reward
                            # This already contains individual reward components if returned by reward function
                            # The reward_extra_infos_dict is already populated from lines 1511-1522
                            ...
                            if "answer" in batch.non_tensor_batch:
                                reward_extra_infos_dict["answer"] = (
                                    batch.non_tensor_batch["answer"].tolist()
                                )
                            # If not found directly, try extracting from extra_info
                                ...
                                for extra_info in extra_infos:
                                    if (
                                        ...
                                        and "answer" in extra_info
                                    ):
                                        answers.append(extra_info["answer"])
                                    else:
                                        answers.append(
                                            None
                                        )  # Use None as placeholder if answer not found
                                if any(a is not None for a in answers):
                                    reward_extra_infos_dict["answer"] = answers

                            # Add problem_id if available
                            if "problem_id" in batch.non_tensor_batch:
                                reward_extra_infos_dict["problem_id"] = (
                                    batch.non_tensor_batch["problem_id"].tolist()
                                )
                            # Also check in extra_info for problem_id
                            elif "extra_info" in batch.non_tensor_batch:
                                extra_infos = batch.non_tensor_batch["extra_info"]
                                problem_ids = []
                                for extra_info in extra_infos:
                                    if (
                                        isinstance(extra_info, dict)
                                        and "problem_id" in extra_info
                                    ):
                                        problem_ids.append(extra_info["problem_id"])
                                    else:
                                        problem_ids.append(None)
                                if any(p is not None for p in problem_ids):
                                    reward_extra_infos_dict["problem_id"] = problem_ids
                            # Add origin_image_path if available
                            if "origin_image_path" in batch.non_tensor_batch:
                                reward_extra_infos_dict["origin_image_path"] = (
                                    batch.non_tensor_batch["origin_image_path"].tolist()
                                )
                            # Also check in extra_info for origin_image_path
                            elif "extra_info" in batch.non_tensor_batch:
                                extra_infos = batch.non_tensor_batch["extra_info"]
                                origin_image_paths = []
                                for extra_info in extra_infos:
                                    if (
                                        isinstance(extra_info, dict)
                                        and "origin_image_path" in extra_info
                                    ):
                                        origin_image_paths.append(
                                            extra_info["origin_image_path"]
                                        )
                                    else:
                                        origin_image_paths.append(None)
                                if any(p is not None for p in origin_image_paths):
                                    reward_extra_infos_dict["origin_image_path"] = (
                                        origin_image_paths
                                    )
                            self._dump_generations(
