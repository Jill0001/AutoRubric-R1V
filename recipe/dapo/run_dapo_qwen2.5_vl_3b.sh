...
PROJECT_NAME='DAPO'
DATA_DIR=/mnt/s3/datasets/verl_training_datasets
SAVE_DIR=/mnt/s3/training_saves/verl_checkpoints
DATA_PATH=$DATA_DIR/ViRL39K
VAR_DATA=$DATA_DIR/val6dataset/validation_data.parquet

EXPERIMENT_NAME='DAPO-Qwen2.5-vl-7B-virl_kl001_5ep'
...
n_resp_per_prompt=8 # gen 8 responses for each prompt
...
# MODEL_PATH=${MODEL_PATH:-"${RAY_DATA_HOME}/models/Qwen2.5-vl-3B"}
# CKPTS_DIR=${CKPTS_DIR:-"${RAY_DATA_HOME}/ckpts/${project_name}/${exp_name}"}
# CKPTS_DIR=/mnt/s3/training_saves/verl_checkpoints/${project_name}/${exp_name}
# mkdir -p ${CKPTS_DIR}
...
for checkpoint_dir in $(find "$LOCAL_SAVE_PATH" -maxdepth 1 -type d -name "global_step_*" | \
    ...
    if [ -d "$checkpoint_dir/actor" ]; then
        ...
        echo ""
        aws s3 sync /mnt/s3/training_saves/verl_checkpoints/DAPO  s3://orby-osu/mengzhao/training_saves/verl_checkpoints/DAPO/  --exclude "**/debug/*" --exclude "**/__pycache__/*" --exclude "**/open_r1.egg-info/*" --exclude "**/*.pt"
